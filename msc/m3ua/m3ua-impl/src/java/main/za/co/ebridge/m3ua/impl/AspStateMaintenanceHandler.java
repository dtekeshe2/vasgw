        ...
        extends MessageHandler
{
    ...
    protected void handleAspUp(ASPUp aspUp)
    {
        ...
        if (aspFactoryImpl.getFunctionality() == Functionality.SGW
                || (aspFactoryImpl.getFunctionality() == Functionality.AS && aspFactoryImpl.getExchangeType() ==
                ExchangeType.DE)
                || (aspFactoryImpl.getFunctionality() == Functionality.IPSP && aspFactoryImpl.getExchangeType() ==
                ExchangeType.DE)
                || (aspFactoryImpl.getFunctionality() == Functionality.IPSP
                && aspFactoryImpl.getExchangeType() == ExchangeType.SE && aspFactoryImpl.getIpspType() == IPSPType
                .SERVER))
        {

            ASPUpAck aspUpAck = (ASPUpAck) this.aspFactoryImpl.messageFactory.createMessage(MessageClass
                            .ASP_STATE_MAINTENANCE,
            ...
            for (Asp asp : this.aspFactoryImpl.aspList)
            {
                ...
                if (AspState.getState(aspPeerFSM.getState().getName()) == AspState.ACTIVE)
                {
                    ...
                    ErrorCode errorCodeObj = this.aspFactoryImpl.parameterFactory.createErrorCode(ErrorCode
                            .Unexpected_Message);
    ...
    protected void handleAspUpAck(ASPUpAck aspUpAck)
    {
        ...
        if (aspFactoryImpl.getFunctionality() == Functionality.AS
                || (aspFactoryImpl.getFunctionality() == Functionality.SGW && aspFactoryImpl.getExchangeType() ==
                ExchangeType.DE)
                || (aspFactoryImpl.getFunctionality() == Functionality.IPSP && aspFactoryImpl.getExchangeType() ==
                ExchangeType.DE)
                || (aspFactoryImpl.getFunctionality() == Functionality.IPSP
                ...
                } else
                {
                    ...
                    try
                    {
                        ...
                        if (aspFactoryImpl.getFunctionality() == Functionality.IPSP)
                        {
                            ...
                            if (asPeerFSM == null)
                            {
                                logger.error(String.format("Received ASPUP_ACK=%s for ASP=%s. But Peer FSM of AS=%s " +
                                                "is null.",
    ...
    protected void handleAspDownAck(ASPDownAck aspUpAck)
    {
        ...
        if (!this.aspFactoryImpl.started)
        {
            ...
            {
                logger.error(String
                        .format("Ungracefully stopping the underlying Association=%s for AspFactory=%s. Atleast one " +
                                        "of the ASP's is Double Exchange and Peer ASP state is still not down",
            ...
            {
                logger.error(
                        String.format("Exception while starting the Association=%s", this.aspFactoryImpl.association
                                .getName()),
                        e);
            }
        } else
