...
import za.co.ebridge.m3ua.impl.parameter.NetworkAppearanceImpl;
import za.co.ebridge.m3ua.impl.parameter.ParameterFactoryImpl;
import za.co.ebridge.m3ua.impl.parameter.RoutingContextImpl;
import za.co.ebridge.m3ua.impl.parameter.TrafficModeTypeImpl;
import za.co.ebridge.mtp.api.RoutingLabelFormat;

public class AsImpl
        implements XMLSerializable, As
{
    ...
    // A set of all the ASP's for this AS
    // protected FastSet<Asp> applicationServerProcesses = new FastSet<Asp>();
    protected List<Asp> applicationServerProcesses = new ArrayList<Asp>();
    ...
    private void initPeerFSM()
    {
        ...
        this.peerFSM.createState(AsState.DOWN.toString())
                .setOnEnter(
                        new SEHPeerAsStateEnterDown(this));

        this.peerFSM.createState(AsState.ACTIVE.toString())
                .setOnEnter(
                        new SEHPeerAsStateEnterActive(this));

        this.peerFSM.createState(AsState.INACTIVE.toString())
                .setOnEnter(
                        new SEHPeerAsStateEnterInactive(this));
        ...
        this.peerFSM.createTransition(TransitionState.AS_STATE_CHANGE_INACTIVE,
                AsState.DOWN.toString(),
                AsState.INACTIVE.toString());

        this.peerFSM.createTransition(TransitionState.ASP_DOWN,
                AsState.DOWN.toString(),
                AsState.DOWN.toString());
        ...
        this.peerFSM.createTransition(TransitionState.AS_STATE_CHANGE_ACTIVE,
                AsState.INACTIVE.toString(),
                AsState.ACTIVE.toString())
                .setHandler(
                        new THPeerAsInActToAct(this, this.peerFSM));

        this.peerFSM.createTransition(TransitionState.ASP_DOWN,
                AsState.INACTIVE.toString(),
                AsState.DOWN.toString())
                .setHandler(
                        new THPeerAsInActToDwn(this, this.peerFSM));
        ...
        this.peerFSM.createTransition(TransitionState.OTHER_ALTERNATE_ASP_ACTIVE,
                AsState.ACTIVE.toString(),
                AsState.ACTIVE.toString())
                .setHandler(
                        new THPeerAsActToActNtfyAltAspAct(this, this.peerFSM));

        this.peerFSM.createTransition(TransitionState.OTHER_INSUFFICIENT_ASP,
                AsState.ACTIVE.toString(),
                AsState.ACTIVE.toString())
                .setHandler(
                        new THPeerAsActToActNtfyInsAsp(this, this.peerFSM));

        this.peerFSM.createTransition(TransitionState.AS_STATE_CHANGE_PENDING,
                AsState.ACTIVE.toString(),
        ...
        this.peerFSM.createTransition(TransitionState.ASP_DOWN,
                ...
                AsState.PENDING.toString())
                .setHandler(
                        new THPeerAsActToPen(this, this.peerFSM));
        ...
        this.peerFSM.createTransition(TransitionState.AS_DOWN,
                AsState.PENDING.toString(),
                AsState.DOWN.toString());

        // As transitions to INACTIVE from PENDING when Pending Timer timesout
        this.peerFSM.createTransition(TransitionState.AS_INACTIVE,
                AsState.PENDING.toString(),
                AsState.INACTIVE.toString());
        ...
        this.peerFSM.createTransition(TransitionState.AS_STATE_CHANGE_ACTIVE,
                ...
                AsState.ACTIVE.toString())
                .setHandler(
                        new THPeerAsPendToAct(this, this.peerFSM));
     ...
     */
    public FastSet<Asp> getAspSet()
    {
//        return this.applicationServerProcesses.unmodifiable();
        FastSet<Asp> set = new FastSet<Asp>();
        for (Asp asp : this.applicationServerProcesses)
        {
            set.add(asp);
        }
     ...
     * {@link AspState#ACTIVE} before state of this As becomes {@link AsState#ACTIVE}
     *
     * @param lb
     */
    ...
    /**
     * Get the minimum number of {@link AspImpl} that should be {@link AspState#ACTIVE}
     ...
     */
    protected void addAppServerProcess(AspImpl aspImpl)
            throws
            Exception
    {
        aspImpl.setAs(this);
        this.applicationServerProcesses.add(aspImpl);
    }

    protected AspImpl removeAppServerProcess(String aspName)
            throws
            Exception
    {
        ...
        for (Asp aspTemp : this.applicationServerProcesses)
        {
            if (aspTemp.getName().equals(aspName))
            {
                aspImpl = (AspImpl) aspTemp;
                break;
            }
        }
        ...
        this.applicationServerProcesses.remove(aspImpl);
        aspImpl.setAs(null);
    ...
    /**
     * write the {@link PayloadData} to underlying {@link AspImpl}.
     * If the state of As is PENDING, the PayloadData is stored in pending queue.
     *
     * @param message
     * @throws IOException
     */
    public void write(PayloadData message)
            throws
            IOException
    {
        ...
        logger.debug("this.functionality : " + this.functionality);
        ...
        {
            fsm = this.peerFSM;
            logger.debug("peerFSM");
        } else
        {
            logger.debug("localFSM");
            fsm = this.localFSM;
            isASPLocalFsm = false;
        }

        int sls = message.getData().getSLS();
        logger.debug("sls : " + sls);

        logger.debug("AsState.getState(fsm.getState().getName()) : " + AsState.getState(fsm.getState().getName()));
        switch (AsState.getState(fsm.getState().getName()))
        {
            ...
            case ACTIVE:
                ...
                // TODO : Algorithm to select correct ASP

                int aspIndex = (sls & this.aspSlsMask);
                aspIndex = (aspIndex >> this.aspSlsShiftPlaces);

                for (int i = 0; i < this.applicationServerProcesses.size(); i++)
                {

                    logger.debug("ACTIVE");

                    AspImpl aspTemp = (AspImpl) this.applicationServerProcesses.get(this.slsVsAspTable[aspIndex++]);

                    FSM aspFsm = null;
                    logger.debug("isASPLocalFsm : " + isASPLocalFsm);
                    if (isASPLocalFsm)
                    {
                    ...
                    logger.debug("AspState.getState(aspFsm.getState().getName()) : " + AspState.getState(aspFsm.getState().getName()));
                    if (AspState.getState(aspFsm.getState().getName()) == AspState.ACTIVE)
                    {
                        logger.debug("ACTIVE ... writing");
                        aspTemp.getAspFactory().write(message);
                        logger.debug("aspFound = true");
                        aspFound = true;
                        break;
                    }
                }
                logger.debug("aspFound : " + aspFound);
                if (!aspFound)
                {
     ...
     */
    protected static final XMLFormat<AsImpl> AS_XML = new XMLFormat<AsImpl>()
    {

        @Override
        public void read(javolution.xml.XMLFormat.InputElement xml, AsImpl asImpl)
                throws
                XMLStreamException
        {
            ...
            asImpl.applicationServerProcesses = xml.get(ASP_LIST, List.class);
            asImpl.init();
        }

        @Override
        public void write(AsImpl asImpl, javolution.xml.XMLFormat.OutputElement xml)
                throws
                XMLStreamException
        {
            ...
            xml.add(asImpl.applicationServerProcesses, ASP_LIST, List.class);
    ...
    public void show(StringBuffer sb)
    {
        ...
        for (Asp aspTemp : this.applicationServerProcesses)
        {
            ...
            sb.append(M3UAOAMMessages.TAB)
                    .append(M3UAOAMMessages.SHOW_ASP_NAME)
                    .append(factory.getName())
                    .append(M3UAOAMMessages.SHOW_STARTED)
                    .append(factory.getStatus());
    ...
    private void resetSlsVsAspTable()
    {
        ...
        {
            if (aspNumber >= this.applicationServerProcesses.size())
            {
                aspNumber = 0;
            }
