        ...
        extends Serializable
{
    ...
    /**
     * Setting this property to true lead that all sent to TCAP messages of this Dialog will be marked as
    ...
    /**
     * Returns the type of the last incoming TCAP primitive (TC-BEGIN, TC-CONTINUE, TC-END or TC-ABORT) It will be
     * equal null if
    ...
    /**
     * This method can be called on timeout of dialog, inside {@link MAPDialogListener#onDialogTimeout(Dialog)}
     * callback. If its
     ...
     * {@link za.co.ebridge.tcap.api.api.tc.dialog.Dialog}
     *
     * @return
     */
    ...
    /**
     * Set ExtentionContainer that will be send in 1) T-BEGIN 2) T-CONTINUE or T-END if it is response to the T-BEGIN
     * 3) T-ABORT
    ...
    /**
     * This is equivalent of MAP User issuing the MAP_DELIMITER Service Request. send() is called to explicitly
     * request the
    ...
    /**
     * This is equivalent of MAP User issuing the MAP_CLOSE Service Request. This service is used for releasing a
     * previously
     * established MAP dialogue. The service may be invoked by either MAP service-user depending on rules defined
     * within the
     * service-user.
     * <p/>
     * <br/>
     ...
     * <br/>
     * <p/>
     * If prearrangedEnd is true, all the Service Primitive added to MAPDialog and not sent yet, will not be sent to
     * peer.
     *
    ...
    /**
     * This method makes the same as send() method. But when invoking it from events of parsing incoming components
     * real sending
     * will occur only when all incoming components events and onDialogDelimiter() or onDialogClose() would be processed
     * <p/>
     * If you are receiving several primitives you can invoke sendDelayed() in several processing components events -
     * the result
    ...
    /**
     * This method makes the same as close() method. But when invoking it from events of parsing incoming components
     * real
     * sending and dialog closing will occur only when all incoming components events and onDialogDelimiter() or
     * onDialogClose()
     * would be processed
     * <p/>
     * If you are receiving several primitives you can invoke closeDelayed() in several processing components events
     * - the
     * result will be sent and the dialog will be closed after onDialogDelimiter() in a single TC-END message
     * <p/>
     * If both of sendDelayed() and closeDelayed() have been invoked TC-END will be issued and the dialog will be
     * closed If
     * sendDelayed() or closeDelayed() were invoked, TC-CONTINUE/TC-END were not sent and abort() or release() are
     * invoked - no
     * TC-CONTINUE/TC-END messages will be sent
     */
    void closeDelayed(boolean prearrangedEnd)
            throws
            MAPException;
     ...
     */
    void refuse(Reason reason)
            throws
            MAPException;

    /**
     * If a MAP user will not answer to an incoming Invoke with Response, Error or Reject components it should invoke
     * this
     ...
     * @throws MAPException
     */
    void sendInvokeComponent(Invoke invoke)
            throws
            MAPException;
     ...
     * @throws MAPException
     */
    void sendReturnResultComponent(ReturnResult returnResult)
            throws
            MAPException;
     ...
     * @throws MAPException
     */
    void sendReturnResultLastComponent(ReturnResultLast returnResultLast)
            throws
            MAPException;
     ...
     * @throws MAPException
     */
    void sendErrorComponent(Long invokeId, MAPErrorMessage mapErrorMessage)
            throws
            MAPException;
     ...
     * @throws MAPException
     */
    void sendRejectComponent(Long invokeId, Problem problem)
            throws
            MAPException;
     ...
     * @throws MAPException
     */
    void resetInvokeTimer(Long invokeId)
            throws
            MAPException;
     ...
     * @return
     */
    int getMessageUserDataLengthOnClose(boolean prearrangedEnd)
            throws
            MAPException;
