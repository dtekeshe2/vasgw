...
import za.co.ebridge.tcap.api.asn.DialogImpl;
...
import za.co.ebridge.map.impl.dialog.MAPAcceptInfoImpl;
import za.co.ebridge.map.impl.dialog.MAPCloseInfoImpl;
import za.co.ebridge.map.impl.dialog.MAPOpenInfoImpl;
import za.co.ebridge.map.impl.dialog.MAPProviderAbortInfoImpl;
import za.co.ebridge.map.impl.dialog.MAPRefuseInfoImpl;
import za.co.ebridge.map.impl.dialog.MAPUserAbortInfoImpl;
import za.co.ebridge.map.impl.errors.MAPErrorMessageFactoryImpl;
import za.co.ebridge.map.impl.errors.MAPErrorMessageImpl;
import za.co.ebridge.map.impl.service.callhandling.MAPServiceCallHandlingImpl;
import za.co.ebridge.map.impl.service.lsm.MAPServiceLsmImpl;
import za.co.ebridge.map.impl.service.mobility.MAPServiceMobilityImpl;
import za.co.ebridge.map.impl.service.oam.MAPServiceOamImpl;
import za.co.ebridge.map.impl.service.pdpContextActivation.MAPServicePdpContextActivationImpl;
import za.co.ebridge.map.impl.service.sms.MAPServiceSmsImpl;
import za.co.ebridge.map.impl.service.supplementary.MAPServiceSupplementaryImpl;
...
import za.co.ebridge.tcap.api.asn.DialogServiceProviderType;
        ...
        implements MAPProvider, TCListener
{
    ...
    private transient List<MAPDialogListener> dialogListeners = new CopyOnWriteArrayList<>();

    /**
     * Session storage
     */
    protected transient Map<Long, MAPDialogImpl> dialogs = new HashMap<>();
    ...
    private final transient MAPSmsTpduParameterFactory mapSmsTpduParameterFactory = new
            MAPSmsTpduParameterFactoryImpl();
    ...
    private final transient MAPServicePdpContextActivation mapServicePdpContextActivation = new
            MAPServicePdpContextActivationImpl(
            this);
    ...
    {
        this.logger = Logger.getLogger(MAPProviderImpl.class.getCanonicalName() + "-" + name);
    ...
    public void onTCBegin(TCBeginIndication tcBeginIndication)
    {
        ...
        logger.debug("tcBeginIndication()");
        logger.debug("tcBeginIndication : " + tcBeginIndication.getApplicationContextName() +
                " oid : " + (tcBeginIndication.getApplicationContextName() != null
                ? tcBeginIndication.getApplicationContextName().getOid() : " ACN = NULL"));

        ApplicationContextName acn = tcBeginIndication.getApplicationContextName();
        Component[] comps = tcBeginIndication.getComponents();

        logger.debug("comps : " + comps);
        ...
        {
            logger.error(String
                    .format("Received TCBeginIndication=%s, both ApplicationContextName and Component[] are null. " +
                                    "Send TC-U-ABORT to peer and not notifying the User",
                            tcBeginIndication));
        ...
        if (acn == null)
        {
            ...
            logger.debug("No application context - MAP Version 1 ? ");
        ...
        {
            logger.debug("Application context is present - MAP Version 2+");
            ...
            if (MAPApplicationContext.getProtocolVersion(acn.getOid()) < 2)
            {
                ...
                // be informed.
                logger.error("Bad version of ApplicationContext if ApplicationContext exists. Must be 2 or greater");
                try
                {
                    this.fireTCAbortProvider(tcBeginIndication.getDialog(), MAPProviderAbortReason.abnormalDialogue,
                            null,
                            false);
                }
                catch (MAPException e)
            ...
            logger.debug("mapAppCtx : " + mapAppCtx);
            if (mapAppCtx == null)
            {
...
//                    || mapAppCtx.getApplicationContextName() == MAPApplicationContextName
// .networkUnstructuredSsContext) {
//
//                logger.error("When parsing TC-BEGIN: userInfo is mandatory for ACN==" + mapAppCtx
// .getApplicationContextName()
//                        + " but not found");
//                try {
//                    this.fireTCAbortProvider(tcBeginIndication.getDialog(), MAPProviderAbortReason
// .abnormalDialogue, null,
//                            false);
//                } catch (MAPException e) {
//                    logger.error("Error while firing TC-U-ABORT. ", e);
//                }
...
//            }
        ...
        } else
        {
            ...
            if (!userInfo.isOid())
            {
                logger.error("When parsing TC-BEGIN: userInfo.isOid() check failed");
                try
                {
            ...
            logger.debug("mapDialAs : " + mapDialAs);
            if (mapDialAs == null)
            {
                logger.error("When parsing TC-BEGIN: Expected MAPDialogueAS.MAP_DialogueAS but is null");
                try
                {
                    this.fireTCAbortProvider(tcBeginIndication.getDialog(), MAPProviderAbortReason.invalidPDU, null,
                            false);
            ...
            logger.debug("userInfo.isAsn : " + userInfo.isAsn());
            if (!userInfo.isAsn())
            {
                logger.error("When parsing TC-BEGIN: userInfo.isAsn() check failed");
                try
                {
                    this.fireTCAbortProvider(tcBeginIndication.getDialog(), MAPProviderAbortReason.invalidPDU, null,
                            false);
            ...
            try
            {
                logger.debug("reading asnData ... ");
                byte[] asnData = userInfo.getEncodeType();
                logger.debug("asnData len : " + (asnData != null ? asnData.length : " NONE "));
                ...
                {
                    logger.error("When parsing TC-BEGIN: MAP-OPEN dialog PDU must be received");
                    try
                    {
                ...
                logger.debug("destReference : " + destReference + ", origReference : " + origReference +
                        ", extensionContainer : " + extensionContainer + ", eriStyle : " + eriStyle +
                        ", eriImsi : " + eriImsi + ", eriVlrNo : " + eriVlrNo);
            }
            catch (AsnException e)
            {
                e.printStackTrace();
                logger.error("AsnException when parsing MAP-OPEN Pdu: " + e.getMessage(), e);
                try
                {
                    this.fireTCAbortProvider(tcBeginIndication.getDialog(), MAPProviderAbortReason.invalidPDU, null,
                            false);
            ...
            {
                e.printStackTrace();
                logger.error("IOException when parsing MAP-OPEN Pdu: " + e.getMessage());
                try
                {
                    this.fireTCAbortProvider(tcBeginIndication.getDialog(), MAPProviderAbortReason.invalidPDU, null,
                            false);
            ...
            {
                e.printStackTrace();
                logger.error("MAPException when parsing MAP-OPEN Pdu: " + e.getMessage());
                try
                {
                    this.fireTCAbortProvider(tcBeginIndication.getDialog(), MAPProviderAbortReason.invalidPDU, null,
                            false);
        ...
        logger.debug("Selecting the MAP service that can perform the ApplicationContext");
        // Selecting the MAP service that can perform the ApplicationContext
        logger.debug("perfSer : " + perfSer);
        if (perfSer == null)
        {
        ...
        if (perfSer == null)
        {
            logger.debug("Unsupported ApplicationContextName is received");
            StringBuffer s = new StringBuffer();
        ...
        logger.debug("perfSer.isActivated() : " + perfSer.isActivated());
        ...
        logger.debug("createNewDialogIncoming ...");
        MAPDialogImpl mapDialogImpl
                = ((MAPServiceBaseImpl) perfSer)
                .createNewDialogIncoming(mapAppCtx,
        ...
        try
        {
            ...
            logger.debug("mapDialogImpl.tcapMessageType : " + mapDialogImpl.tcapMessageType);
            ...
            logger.debug("mapDialogImpl.getState() : " + mapDialogImpl.getState());
            mapDialogImpl.delayedAreaState = MAPDialogImpl.DelayedAreaState.No;

            logger.debug("eriStyle : " + eriStyle);
            ...
            logger.debug("Now let us decode the Components : " + comps + " ...");
            ...
            logger.debug("this.deliverDialogDelimiter() ...");
            this.deliverDialogDelimiter(mapDialogImpl);

            logger.debug("finishComponentProcessingState() ...");
            finishComponentProcessingState(mapDialogImpl);

            logger.debug("this.getTCAPProvider().getPreviewMode() : " + this.getTCAPProvider().getPreviewMode());
            if (this.getTCAPProvider().getPreviewMode())
            {
                logger.debug("setUpperDialog");
        ...
        {
            logger.debug("mapDialogImpl.getTcapDialog().getDialogLock().unlock()");
    ...
    private void finishComponentProcessingState(MAPDialogImpl mapDialogImpl)
    {
        ...
        {
            logger.error("Error while finishComponentProcessingState, delayedAreaState=" + mapDialogImpl
                    .delayedAreaState, e);
        }
    ...
    public void onTCContinue(TCContinueIndication tcContinueIndication)
    {
        ...
        {
            logger.error("MAP Dialog not found for Dialog Id " + tcapDialog.getLocalDialogId());
            try
            {
                ...
                if (userInfo != null)
                {
                    ...
                    if (userInfo.isOid())
                    {
                        ...
                        if (mapDialAs != null && userInfo.isAsn())
                        {
                            ...
                            {
                                e.printStackTrace();
                                logger.error("AsnException when parsing MAP-ACCEPT Pdu: " + e.getMessage(), e);
                                return;
                            }
                            catch (IOException e)
                            {
                                e.printStackTrace();
                                logger.error("IOException when parsing MAP-ACCEPT Pdu: " + e.getMessage(), e);
                            }
                            catch (MAPParsingComponentException e)
                            {
                                e.printStackTrace();
                                logger.error("MAPException when parsing MAP-ACCEPT Pdu: " + e.getMessage(), e);
                            }
                        }
                    }
                }
            ...
            } else
            {
                ...
                if (mapDialogImpl.getState() == MAPDialogState.INITIAL_SENT)
                {
                    ...
                    if (acn == null)
                    {
                        ...
                        if (mapDialogImpl.getApplicationContext().getApplicationContextVersion() !=
                                MAPApplicationContextVersion.version1)
                        {

                            logger.error(String.format(
                                    "Received first TC-CONTINUE for MAPDialog=%s. But no application-context-name " +
                                            "included",
                                    mapDialogImpl));
                            try
                            {
                            ...
                            this.deliverDialogProviderAbort(mapDialogImpl, MAPAbortProviderReason
                                            .AbnormalMAPDialogueLocal,
                    ...
                    } else
                    {
                        ...
                        if (mapAcn == null || !mapAcn.equals(mapDialogImpl.getApplicationContext()))
                        {
                            ...
                            logger.error(String.format("Received first TC-CONTINUE. MAPDialog=%s. But " +
                                            "MAPApplicationContext=%s",
                ...
                } else
                {
                    ...
                    logger.error(String.format("Received TC-CONTINUE. MAPDialog=%s. But state is neither InitialSent " +
                                    "or Active",
                            mapDialogImpl));
                }
                ...
                if (userInfo != null)
                {
                    ...
                    if (userInfo.isOid())
                    {
                        ...
                        if (mapDialAs != null && userInfo.isAsn())
                        {
                            ...
                            {
                                e.printStackTrace();
                                logger.error("AsnException when parsing MAP-ACCEPT/MAP-CLOSE Pdu: " + e.getMessage(),
                                        e);
                                return;
                            }
                            catch (IOException e)
                            {
                                e.printStackTrace();
                                logger.error("IOException when parsing MAP-ACCEPT/MAP-CLOSE Pdu: " + e.getMessage(), e);
                            }
                            catch (MAPParsingComponentException e)
                            {
                                e.printStackTrace();
                                logger.error("MAPException when parsing MAP-ACCEPT/MAP-CLOSE Pdu: " + e.getMessage(),
                                        e);
                            }
                        }
                    }
                ...
                if (mapDialogImpl.getState() == MAPDialogState.INITIAL_SENT)
                {
                    ...
                    if (acn == null)
                    {
                                ...
                                MAPApplicationContextVersion.version1)
                        {
                            ...
                            {
                                logger.error(String.format(
                                        "Received first TC-END for MAPDialog=%s. But no application-context-name " +
                                                "included",
                                        mapDialogImpl));
                    ...
                    } else
                    {
                        ...
                        if (mapAcn == null || !mapAcn.equals(mapDialogImpl.getApplicationContext()))
                        {
                            logger.error(String.format("Received first TC-END. MAPDialog=%s. But " +
                                            "MAPApplicationContext=%s",
                                    mapDialogImpl, mapAcn));
                    ...
                    if (userInfo != null)
                    {
                        ...
                        if (userInfo.isOid())
                        {
                            ...
                            if (mapDialAs != null && userInfo.isAsn())
                            {
                                ...
                                {
                                    e.printStackTrace();
                                    logger.error("AsnException when parsing MAP-ACCEPT/MAP-CLOSE Pdu: " + e
                                            .getMessage(), e);
                                    return;
                                }
                                catch (IOException e)
                                {
                                    e.printStackTrace();
                                    logger.error("IOException when parsing MAP-ACCEPT/MAP-CLOSE Pdu: " + e.getMessage
                                            (), e);
                                }
                                catch (MAPParsingComponentException e)
                                {
                                    e.printStackTrace();
                                    logger.error("MAPException when parsing MAP-ACCEPT/MAP-CLOSE Pdu: " + e
                                            .getMessage(), e);
                                }
                            }
                        }
    ...
    public void onInvokeTimeout(Invoke invoke)
    {
        ...
        MAPDialogImpl mapDialogImpl = (MAPDialogImpl) this.getMAPDialog(((InvokeImpl) invoke).getDialog()
                .getLocalDialogId());
    ...
    public void onTCUserAbort(TCUserAbortIndication tcUserAbortIndication)
    {
        ...
        try
        {
            ...
            if (userInfo != null)
            {
                ...
                if (!userInfo.isOid())
                {
                    logger.error("When parsing TCUserAbortIndication indication: userInfo.isOid() check failed");
                ...
                } else
                {
                    ...
                    if (mapDialAs == null)
                    {
                        logger.error("When parsing TCUserAbortIndication indication: userInfo.getOidValue() must be " +
                                "userInfoMAPDialogueAS.MAP_DialogueAS");
                        parsePduResult = ParsePduResult.BadUserInfo;
                    } else if (!userInfo.isAsn())
                    {

                        logger.error("When parsing TCUserAbortIndication indication: userInfo.isAsn() check failed");
                        parsePduResult = ParsePduResult.BadUserInfo;
                    } else
                    {
                        ...
                        try
                        {
                            ...
                            switch (tag)
                            {
                                ...
                                default:
                                    logger.error("When parsing TCUserAbortIndication indication: " +
                                            "userInfogetEncodeType().Tag must be either MAP_REFUSE_INFO_TAG or " +
                                            "MAP_USER_ABORT_INFO_TAG or MAP_PROVIDER_ABORT_INFO_TAG");
                                    parsePduResult = ParsePduResult.BadUserInfo;
                                    break;
                            }
                        ...
                        {
                            logger.error("When parsing TCUserAbortIndication indication: AsnException" + e.getMessage
                                    (), e);
                            parsePduResult = ParsePduResult.BadUserInfo;
                        }
                        catch (IOException e)
                        {
                            logger.error("When parsing TCUserAbortIndication indication: IOException" + e.getMessage
                                    (), e);
                            parsePduResult = ParsePduResult.BadUserInfo;
                        }
                        catch (MAPParsingComponentException e)
            ...
            if (tcUserAbortIndication.IsAareApdu())
            {
                ...
                if (resultSourceDiagnostic != null)
                {
                    ...
                    } else if (resultSourceDiagnostic.getDialogServiceProviderType() == DialogServiceProviderType
                            .NoCommonDialogPortion)
                    {
                        if (mapDialogImpl.getState() == MAPDialogState.INITIAL_SENT)
                        {
                            // NoCommonDialogPortion
                            this.deliverDialogReject(mapDialogImpl, MAPRefuseReason.PotentialVersionIncompatibility,
                                    null, null);
            ...
            switch (compType)
            {
                case Invoke:
                {
                    ...
                    if (linkedId != null)
                    {
                        ...
                        long[] lstInv = perfSer.getLinkedOperationList(linkedInvoke.getOperationCode()
                ...
                {
                    Reject rej = (Reject) c;
                    perfSer.deliverRejectComponent(mapDialogImpl, rej.getInvokeId(), rej.getProblem(), rej
                            .isLocalOriginated());
    ...
    {
        logger.debug("deliverDialogDelimiter : " + this.dialogListeners);
    ...
    private void deliverDialogRequest(
            MAPDialog mapDialog, AddressString destReference, AddressString origReference,
            MAPExtensionContainer extensionContainer)
    {
        logger.debug("deliverDialogRequest() : " + this.dialogListeners);
        for (MAPDialogListener listener : this.dialogListeners)
        {
            logger.debug("listener : " + listener);
    ...
    {
        logger.debug("deliverDialogAccept");
        for (MAPDialogListener listener : this.dialogListeners)
        {
    ...
    private void deliverDialogReject(
            MAPDialog mapDialog, MAPRefuseReason refuseReason,
    ...
    {
        logger.debug("deliverDialogClose");
        for (MAPDialogListener listener : this.dialogListeners)
        {
    ...
    private void deliverDialogProviderAbort(
            MAPDialog mapDialog, MAPAbortProviderReason abortProviderReason,
    ...
    private void deliverDialogUserAbort(
            MAPDialog mapDialog, MAPUserAbortChoice userReason,
            MAPExtensionContainer extensionContainer)
    {
        for (MAPDialogListener listener : this.dialogListeners)
        {
    ...
    protected void fireTCBegin(
            Dialog tcapDialog, ApplicationContextName acn, AddressString destReference,
            AddressString origReference, MAPExtensionContainer mapExtensionContainer, boolean isEriStyle, IMSI imsiEri,
            AddressString vlrNoEri, boolean returnMessageOnError)
            throws
            MAPException
    {
        logger.debug("fireTCBegin");
        if (this.getTCAPProvider().getPreviewMode())
        {
            return;
        }
    ...
    protected TCBeginRequest encodeTCBegin(
            Dialog tcapDialog, ApplicationContextName acn, AddressString destReference,
    ...
    protected void fireTCContinue(
            Dialog tcapDialog, Boolean sendMapAcceptInfo, ApplicationContextName acn,
            MAPExtensionContainer mapExtensionContainer, boolean returnMessageOnError)
            throws
            MAPException
    {

        logger.debug("fireTCContinue, tcapDialog : localAddress " + tcapDialog.getLocalAddress().getGlobalTitle());
        logger.debug("fireTCContinue, tcapDialog : remoteAddress " + tcapDialog.getRemoteAddress().getGlobalTitle());
        logger.debug("getPreviewMode() " + this.getTCAPProvider().getPreviewMode());
        if (this.getTCAPProvider().getPreviewMode())
        {
            return;
        }

        logger.debug("encodeTCContinue");
        TCContinueRequest tcContinueReq = encodeTCContinue(tcapDialog, sendMapAcceptInfo, acn, mapExtensionContainer);
        logger.debug("done returnMessageOnError : " + returnMessageOnError);
        if (returnMessageOnError)
            tcContinueReq.setReturnMessageOnError(true);

        try
        {
            logger.debug("tcapDialog.send()");
            tcapDialog.send(tcContinueReq);
            logger.debug("sent");
        }
        catch (TCAPSendException e)
        {
            logger.debug("TCAPSendException : " + e.getMessage());
            throw new MAPException(e.getMessage(), e);
        }
    }

    protected TCContinueRequest encodeTCContinue(
            Dialog tcapDialog, Boolean sendMapAcceptInfo, ApplicationContextName acn,
            ...
            MAPException
    {
        ...
        logger.debug("encodeTCContinue : " + acn);

        // we do not set ApplicationContextName if MAP Version 1
        if (acn != null && MAPApplicationContext.getProtocolVersion(acn.getOid()) > 1)
        {
            logger.debug("setApplicationContextName ... MAP Version 1");
            tcContinueReq.setApplicationContextName(acn);
        }

        if (sendMapAcceptInfo && mapExtensionContainer != null && MAPApplicationContext.getProtocolVersion(acn.getOid
                ()) > 1)
        {

            logger.debug("setUserInformation");
    ...
    protected void fireTCEnd(
            Dialog tcapDialog, Boolean sendMapCloseInfo, boolean prearrangedEnd, ApplicationContextName acn,
            MAPExtensionContainer mapExtensionContainer, boolean returnMessageOnError)
            throws
            MAPException
    {

        logger.debug("fireTCEnd");
        if (this.getTCAPProvider().getPreviewMode())
        {
            return;
        }
    ...
    protected TCEndRequest encodeTCEnd(
            Dialog tcapDialog, Boolean sendMapCloseInfo, boolean prearrangedEnd,
            ApplicationContextName acn, MAPExtensionContainer mapExtensionContainer)
            throws
            MAPException
    {
        TCEndRequest endRequest = this.getTCAPProvider().getDialogPrimitiveFactory().createEnd(tcapDialog);

        logger.debug("encodeTCEnd");
        if (!prearrangedEnd)
        {
        ...
        if (sendMapCloseInfo && mapExtensionContainer != null && MAPApplicationContext.getProtocolVersion(acn.getOid
                ()) > 1)
        {
            MAPAcceptInfoImpl mapAccept = new MAPAcceptInfoImpl();
            mapAccept.setExtensionContainer(mapExtensionContainer);
     ...
     * @throws MAPException
     */
    private void fireTCAbortACNNotSupported(
            Dialog tcapDialog, MAPExtensionContainer mapExtensionContainer,
     ...
     * @throws MAPException
     */
    protected void fireTCAbortRefused(
            Dialog tcapDialog, Reason reason, MAPExtensionContainer mapExtensionContainer,
     ...
     * @throws MAPException
     */
    protected void fireTCAbortUser(
            Dialog tcapDialog, MAPUserAbortChoice mapUserAbortChoice,
            MAPExtensionContainer mapExtensionContainer, boolean returnMessageOnError)
            throws
            MAPException
    {
     ...
     * @throws MAPException
     */
    protected void fireTCAbortProvider(
            Dialog tcapDialog, MAPProviderAbortReason mapProviderAbortReason,
            MAPExtensionContainer mapExtensionContainer, boolean returnMessageOnError)
            throws
            MAPException
    {
     ...
     * @throws MAPException
     */
    protected void fireTCAbortV1(Dialog tcapDialog, boolean returnMessageOnError)
            throws
            MAPException
    {
