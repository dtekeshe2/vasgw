...
public class InsertSubscriberDataRequestImpl
        extends MobilityMessageImpl
        implements InsertSubscriberDataRequest
{
    ...
    public InsertSubscriberDataRequestImpl(
            long mapProtocolVersion, IMSI imsi, ISDNAddressString msisdn, Category category,
            ...
            AsnException
    {
                ...
                case Tag.CLASS_CONTEXT_SPECIFIC:
                    switch (tag)
                    {
                        ...
                        case _TAG_msisdn:
                            if (!ais.isTagPrimitive())
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ".msisdn: is not primitive", MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                        ...
                        case _TAG_category:
                            if (!ais.isTagPrimitive())
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ".category: is not primitive", MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                        ...
                        case _TAG_bearerServiceList:
                            ...
                            if (this.bearerServiceList.size() < 1 || this.bearerServiceList.size() > 50)
                            {
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        ...
                                        + this.bearerServiceList.size(), MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                            }
                            break;
                        case _TAG_teleserviceList:
                            ...
                            if (this.teleserviceList.size() < 1 || this.teleserviceList.size() > 20)
                            {
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        ...
                                        + this.teleserviceList.size(), MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                            }
                            break;
                        case _TAG_provisionedSS:
                            ...
                            if (this.provisionedSS.size() < 1 || this.provisionedSS.size() > 30)
                            {
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        ...
                                        + this.provisionedSS.size(), MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                            }
                            break;
                        case _TAG_odb_Data:
                            if (ais.isTagPrimitive())
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ".odbData: is primitive", MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                        ...
                        case _TAG_vbsSubscriptionData:
                            ...
                            while (true)
                            {
                                ...
                                int tag4 = ais4.readTag();
                                if (tag4 != Tag.SEQUENCE || ais4.getTagClass() != Tag.CLASS_UNIVERSAL || ais4
                                        .isTagPrimitive())
                                    throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                            ...
                            if (this.vbsSubscriptionData.size() < 1 || this.vbsSubscriptionData.size() > 50)
                            {
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        ...
                                        + this.vbsSubscriptionData.size(), MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                            }
                            break;
                        ...
                        case _TAG_lsaInformation:
                            if (ais.isTagPrimitive())
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ".lsaInformation: is primitive", MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                        ...
                        case _TAG_lcsInformation:
                            if (ais.isTagPrimitive())
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ".lcsInformation: is primitive", MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                        ...
                        case _TAG_istAlertTimer:
                            ...
                            {
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ": Parameter istAlertTimer must be from 15 to 255, parsed: " + this
                                        .istAlertTimer,
                                        MAPParsingComponentExceptionReason.MistypedParameter);
                            }
                            break;
                        ...
                        case _TAG_mc_SS_Info:
                            if (ais.isTagPrimitive())
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ".mcSsInfo: is primitive", MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                        ...
                        case _TAG_sgsn_Number:
                            if (!ais.isTagPrimitive())
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ".sgsnNumber: is primitive", MAPParsingComponentExceptionReason
                                        .MistypedParameter);
        ...
        if (num == 0)
            throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                    + ": Needs at least 1 parameter, found " + num, MAPParsingComponentExceptionReason
                    .MistypedParameter);
    }
            ...
            MAPException
    {
        ...
        if (this.bearerServiceList != null && (this.bearerServiceList.size() < 1 || this.bearerServiceList.size() > 50))
            throw new MAPException("bearerServiceList size must be from 1 to 50, found: " + this.bearerServiceList
                    .size());
        ...
        if (this.vbsSubscriptionData != null && (this.vbsSubscriptionData.size() < 1 || this.vbsSubscriptionData.size
                () > 50))
            throw new MAPException("vbsSubscriptionData size must be from 1 to 50, found: " + this
                    .vbsSubscriptionData.size());

        if (this.vgcsSubscriptionData != null
                && (this.vgcsSubscriptionData.size() < 1 || this.vgcsSubscriptionData.size() > 50))
            throw new MAPException("vgcsSubscriptionData size must be from 1 to 50, found: " + this
                    .vgcsSubscriptionData.size());
        ...
        if (this.subscriberStatus != null)
        {
            ...
            {
                throw new MAPException("IOException while encoding " + _PrimitiveName + " parameter " +
                        "subscriberStatus", e);
            }
            catch (AsnException e)
            {
        ...
        if (mapProtocolVersion >= 3)
        {
            ...
            if (this.networkAccessMode != null)
            {
                try
                {
                    asnOs.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_networkAccessMode, this.networkAccessMode
                            .getCode());
                }
                catch (IOException e)
                {
            ...
            {
                ((LSAInformationImpl) this.lsaInformation).encodeAll(asnOs, Tag.CLASS_CONTEXT_SPECIFIC,
                        _TAG_lsaInformation);
            }
            ...
            {
                ((LCSInformationImpl) this.lcsInformation).encodeAll(asnOs, Tag.CLASS_CONTEXT_SPECIFIC,
                        _TAG_lcsInformation);
            }
            ...
            {
                ((SGSNCAMELSubscriptionInfoImpl) this.sgsnCamelSubscriptionInfo).encodeAll(asnOs, Tag
                                .CLASS_CONTEXT_SPECIFIC,
                        _TAG_sgsn_CAMEL_SubscriptionInfo);
            }

            if (this.chargingCharacteristics != null)
                ((ChargingCharacteristicsImpl) this.chargingCharacteristics).encodeAll(asnOs, Tag
                                .CLASS_CONTEXT_SPECIFIC,
            ...
            {
                ((ISDNAddressStringImpl) this.sgsnNumber).encodeAll(asnOs, Tag.CLASS_CONTEXT_SPECIFIC,
                        _TAG_sgsn_Number);
            }
            ...
            if (this.vplmnLIPAAllowed)
            {
                ...
                {
                    throw new MAPException("IOException when encoding " + _PrimitiveName + " parameter " +
                            "vplmnLIPAAllowed: "
                            + e.getMessage(), e);
                }
                catch (AsnException e)
                {
                    throw new MAPException("AsnException when encoding " + _PrimitiveName + " parameter " +
                            "vplmnLIPAAllowed: "
                            + e.getMessage(), e);
                }
            }
