...
public class PSSubscriberStateImpl
        implements PSSubscriberState, MAPAsnPrimitive
{
        ...
        } else if (ansIS.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC)
        {
            switch (ansIS.getTag())
            {
                ...
                case _ID_ps_AttachedNotReachableForPaging:
                    if (!ansIS.isTagPrimitive())
                        throw new MAPParsingComponentException(
                                "Error while decoding psAttachedNotReachableForPaging choice: Parameter is not " +
                                        "primitive",
                                MAPParsingComponentExceptionReason.MistypedParameter);
    ...
    private void decodePdpContextInfoList(AsnInputStream ansIS, int length)
            throws
            AsnException,
            IOException,
            MAPParsingComponentException
            ...
            MAPException
    {
        ...
        try
        {
            ...
            if ((this.choice == PSSubscriberStateChoice.psPDPActiveNotReachableForPaging || this.choice ==
                    PSSubscriberStateChoice.psPDPActiveReachableForPaging)
                    && this.pdpContextInfoList == null)
                throw new MAPException(
                        "Error while encoding the "
                                + _PrimitiveName
                                + ": for choice psPDPActiveNotReachableForPaging or psPDPActiveReachableForPaging - " +
                                "pdpContextInfoList must not be null");
