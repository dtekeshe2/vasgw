...
public class SubscriberInfoImpl
        implements SubscriberInfo, MAPAsnPrimitive
{
        ...
        while (true)
        {
            ...
            if (ais.getTagClass() == Tag.CLASS_CONTEXT_SPECIFIC)
            {
                switch (tag)
                {
                    ...
                    case _ID_imei:
                        if (!ais.isTagPrimitive())
                            throw new MAPParsingComponentException("Error while decoding IMEI: Parameter is not " +
                                    "primitive",
                                    MAPParsingComponentExceptionReason.MistypedParameter);
                        imei = new IMEIImpl();
                    ...
                    case _ID_gprsMSClass:
                        if (ais.isTagPrimitive())
                            throw new MAPParsingComponentException("Error while decoding gprsMSClass: Parameter is " +
                                    "primitive",
                                    MAPParsingComponentExceptionReason.MistypedParameter);
                    ...
                    case _ID_mnpInfoRes:
                        if (ais.isTagPrimitive())
                            throw new MAPParsingComponentException("Error while decoding mnpInfoRes: Parameter is " +
                                    "primitive",
                                    MAPParsingComponentExceptionReason.MistypedParameter);
            ...
            MAPException
    {
        try
        {
            ...
            if (this.locationInformationGPRS != null)
                ((LocationInformationGPRSImpl) this.locationInformationGPRS).encodeAll(asnOs, Tag
