...
public class OBcsmCamelTdpCriteriaImpl
        ...
        implements OBcsmCamelTdpCriteria
{
                        ...
                        case _TAG_oCauseValueCriteria:
                            ...
                            if (this.oCauseValueCriteria.size() < 1 || this.oCauseValueCriteria.size() > 5)
                            {
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        ...
                                        + this.oCauseValueCriteria.size(), MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                            }
                            break;
            ...
            MAPException
    {
        ...
        if (this.basicServiceCriteria != null && (this.basicServiceCriteria.size() < 1 || this.basicServiceCriteria
                .size() > 5))
        {
            throw new MAPException("Error while encoding " + _PrimitiveName
                    + ": Parameter basicServiceCriteria size must be from 1 to 5, found: " + this
                    .basicServiceCriteria.size());
        }

        if (this.oCauseValueCriteria != null && (this.oCauseValueCriteria.size() < 1 || this.oCauseValueCriteria.size
                () > 5))
        {
            throw new MAPException("Error while encoding " + _PrimitiveName
                    + ": Parameter oCauseValueCriteria size must be from 1 to 5, found: " + this.oCauseValueCriteria
                    .size());
        }
        ...
        try
        {
            ...
            if (this.destinationNumberCriteria != null)
                ((DestinationNumberCriteriaImpl) this.destinationNumberCriteria).encodeAll(asnOs, Tag
