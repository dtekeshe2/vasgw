...
import za.co.ebridge.map.api.smstpdu.SmsTpduType;
...
import za.co.ebridge.map.api.smstpdu.ValidityPeriodFormat;

public class SmsSubmitTpduImpl
        extends SmsTpduImpl
        implements SmsSubmitTpdu
{
    ...
    public SmsSubmitTpduImpl(byte[] data, Charset gsm8Charset)
            throws
            MAPException
    {
        this();
        ...
        if (this.messageReference == -1)
            throw new MAPException("Error creating a new SmsSubmitTpdu instance: messageReference field has not been " +
                    "found");
        ...
        bt = stm.read();
        if (bt == -1)
            throw new MAPException("Error creating a new SmsSubmitTpdu instance: protocolIdentifier field has not " +
                    "been found");
        this.protocolIdentifier = new ProtocolIdentifierImpl(bt);

        bt = stm.read();
        if (bt == -1)
            throw new MAPException("Error creating a new SmsSubmitTpdu instance: dataCodingScheme field has not been " +
                    "found");
        ...
        switch (this.validityPeriodFormat)
        {
            case fieldPresentRelativeFormat:
                ...
                if (bt == -1)
                    throw new MAPException(
                            "Error creating a new SmsSubmitTpdu instance: " +
                                    "validityPeriodFormat-fieldPresentEnhancedFormat field has not been found");
            ...
            case fieldPresentEnhancedFormat:
                ...
                {
                    throw new MAPException(
                            "IOException while creating a new SmsSubmitTpdu instance - decoding " +
                                    "validityPeriodFormat-fieldPresentAbsoluteFormat: "
                                    + e.getMessage(), e);
                }
        ...
        if (this.userDataLength == -1)
            throw new MAPException("Error creating a new SmsDeliverTpduImpl instance: userDataLength field has not " +
                    "been found");
            ...
            MAPException
    {
        ...
        if (this.destinationAddress == null || this.protocolIdentifier == null || this.userData == null)
            throw new MAPException(
                    "Error encoding a SmsSubmitTpdu: destinationAddress, protocolIdentifier and userData must not be " +
                            "null");
