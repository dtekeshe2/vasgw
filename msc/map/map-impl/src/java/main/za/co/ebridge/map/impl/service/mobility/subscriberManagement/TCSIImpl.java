            ...
            switch (num)
            {
                case 0:
                    ...
                    if (this.tBcsmCamelTDPDataList.size() < 1 || this.tBcsmCamelTDPDataList.size() > 10)
                        throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                + ".tBcsmCamelTDPDataList: tBcsmCamelTDPData elements count must be from 1 to 10, " +
                                "found: "
                                + this.tBcsmCamelTDPDataList.size(), MAPParsingComponentExceptionReason
                                .MistypedParameter);
                    break;
            ...
            MAPException
    {
        try
        {
            if (this.tBcsmCamelTDPDataList == null)
                throw new MAPException("Error while encoding" + _PrimitiveName + ": tBcsmCamelTDPDataList must not be" +
                        " null");
            if (this.tBcsmCamelTDPDataList.size() < 1 || this.tBcsmCamelTDPDataList.size() > 10)
                throw new MAPException("Error while encoding" + _PrimitiveName
                        + ": tBcsmCamelTDPDataList size must be from 1 to 10, found: " + this.tBcsmCamelTDPDataList
                        .size());
