        ...
        implements APNConfiguration
{
    ...
    public APNConfigurationImpl(
            int contextId, PDNType pDNType, PDPAddress servedPartyIPIPv4Address, APN apn,
                ...
                case Tag.CLASS_CONTEXT_SPECIFIC:
                {
                    switch (tag)
                    {
                        ...
                        case _TAG_ambr:
                            if (ais.isTagPrimitive())
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        + ".ambr: Parameter is primitive", MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                        ...
                        case _TAG_specificAPNInfoList:
                            ...
                            if (this.specificAPNInfoList.size() < 1 || this.specificAPNInfoList.size() > 50)
                            {
                                throw new MAPParsingComponentException("Error while decoding " + _PrimitiveName
                                        ...
                                        + this.specificAPNInfoList.size(), MAPParsingComponentExceptionReason
                                        .MistypedParameter);
                            }
                            break;
            ...
            MAPException
    {
        ...
        if (this.pDNType == null)
        {
            throw new MAPException("Error while encoding " + _PrimitiveName + " the mandatory parameter pDNType is " +
                    "not defined");
        }
        ...
        if (this.specificAPNInfoList != null && (this.specificAPNInfoList.size() < 1 || this.specificAPNInfoList.size
                () > 50))
        {
            throw new MAPException("Error while encoding " + _PrimitiveName
                    + ": Parameter specificAPNInfoList size must be from 1 to 50, found: " + this.specificAPNInfoList
                    .size());
        }
        ...
        try
        {
            ...
            ((EPSQoSSubscribedImpl) this.ePSQoSSubscribed).encodeAll(asnOs, Tag.CLASS_CONTEXT_SPECIFIC,
                    _TAG_ePSQoSSubscribed);
            ...
            if (this.pdnGwAllocationType != null)
                asnOs.writeInteger(Tag.CLASS_CONTEXT_SPECIFIC, _TAG_pdnGwAllocationType, this.pdnGwAllocationType
                        .getCode());
