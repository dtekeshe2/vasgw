        ...
        implements ConcatenatedShortMessagesIdentifier
{
     ...
     * @param reference           Concatenated short message reference number. This octet shall contain a modulo 256
     *                            counter indicating
     *                            the reference number for a particular concatenated short message. This reference
     *                            number shall remain constant for
     *                            every short message which makes up a particular concatenated short message.
     * @param mesageSegmentCount  Maximum number of short messages in the concatenated short message. This octet
     *                            shall contain a
     *                            value in the range 0 to 255 indicating the total number of short messages within
     *                            the concatenated short message.
     *                            The value shall start at 1 and remain constant for every short message which makes
     *                            up the concatenated short
     *                            message. If the value is zero then the receiving entity shall ignore the whole
     *                            Information Element.
     * @param mesageSegmentNumber Sequence number of the current short message. This octet shall contain a value in
     *                            the range 0
     *                            to 255 indicating the sequence number of a particular short message within the
     *                            concatenated short message. The
     *                            value shall start at 1 and increment by one for every short message sent within the
     *                            concatenated short message. If
     *                            the value is zero or the value is greater than the value in octet 2 then the
     *                            receiving entity shall ignore the
     *                            whole Information Element.
     */
    public ConcatenatedShortMessagesIdentifierImpl(
            boolean referenceIs16bit, int reference, int mesageSegmentCount,
    ...
    public ConcatenatedShortMessagesIdentifierImpl(byte[] encodedInformationElementData)
    {
        ...
        } else
        {
            ...
            this.reference = (encodedInformationElementData[0] & 0xFF) + ((encodedInformationElementData[1] & 0xFF)
                    << 8);
