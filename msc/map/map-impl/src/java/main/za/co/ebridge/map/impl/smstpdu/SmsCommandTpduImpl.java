...
public class SmsCommandTpduImpl
        extends SmsTpduImpl
        implements SmsCommandTpdu
{
    ...
    public SmsCommandTpduImpl(
            boolean statusReportRequest, int messageReference, ProtocolIdentifier protocolIdentifier,
            ...
            MAPException
    {
        ...
        if (this.messageReference == -1)
            throw new MAPException("Error creating a new SmsCommandTpdu instance: messageReference field has not been" +
                    " found");

        bt = stm.read();
        if (bt == -1)
            throw new MAPException("Error creating a new SmsCommandTpdu instance: protocolIdentifier field has not " +
                    "been found");
        this.protocolIdentifier = new ProtocolIdentifierImpl(bt);

        bt = stm.read();
        if (bt == -1)
            throw new MAPException("Error creating a new SmsCommandTpdu instance: commandType field has not been " +
                    "found");
        this.commandType = new CommandTypeImpl(bt);

        this.messageNumber = stm.read();
        if (this.messageNumber == -1)
            throw new MAPException("Error creating a new SmsCommandTpdu instance: messageNumber field has not been " +
                    "found");
        ...
        if (this.commandDataLength == -1)
            throw new MAPException("Error creating a new SmsCommandTpdu instance: commandDataLength field has not " +
                    "been found");
            ...
            MAPException
    {
        ...
        if (this.protocolIdentifier == null || this.commandData == null || commandType == null || destinationAddress
                == null)
            throw new MAPException(
                    "Error encoding a SmsCommandTpdu: protocolIdentifier, messageNumber, destinationAddress and " +
                            "commandData must not be null");
