...
/**
 * Utility class to dump the raw data as hex dump
 */
public class HexTools
{
    ...
    /**
     * Generate a hex frame from the provided byte buffer
     * The Buffer will be returned to it's original state
     ...
     *
     * @param buffer
     * @param length print up to number of bytes. 0 means print all remaining.
     * @param printAddress Should the printout start from the ByteBuffer current position.
     * @return StringBuffer with the hex frame.
     */
     ...
     * Generate a hex frame from the provided byte buffer
     * The Buffer will be returned to it's original state
     * Start address references the initial address of the provided buffer
     *
     * @param startAddress address of the provided data chunk.
     * @return StringBuffer with the hex frame.
     */
    ...
    /**
     * Prints a hex frame from the byte array of the provided String.
     * Start address references the initial address of the provided string
     *
     * @param data
     * @param startAddress address of the provided data chunk.
     * @return StringBuffer with the hex frame.
     ...
     * 20: 20 21 22 -- -- -- -- -- -- -- -- -- -- -- -- -- | ...
     *
     * @param data String of bytes to dump
     * @return StringBuffer with the hex frame
     */
    ...
    /**
     * Prints bytes in hex form, 16 bytes per line.
     * startAddress sets the first "address" the given array is printed from
     *
     * In this example the start address were 0x3456 on a 35 byte array
     *
    ...
    public static StringBuffer dump(byte[] data, int startAddress)
    {
        ...
        sb.append("Start: ")
                .append(startAddress)
                .append(" (0x")
                .append(HexTools.toHex(startAddress, hexDigitCount * 4, HexTools.SPACING_NONE))
                .append(")  End: ")
                .append(startAddress + dataLength - 1)
                .append(" (0x")
                .append(HexTools.toHex(startAddress + dataLength - 1, hexDigitCount * 4, HexTools.SPACING_NONE))
                .append(")  Length: ")
                .append(dataLength)
                .append(" (0x")
                .append(HexTools.toHex(dataLength, hexDigitLengthCount * 4, HexTools.SPACING_NONE))
                .append(")\n");
        ...
        for (int i = 0; i < dataLength; i++)
        {
            if ((i + startAddressMod) % 16 == 0 || i == 0)
            {
                if (i == 0)
                {
                    sb.append(HexTools.toHex(i + startAddressMain, hexDigitCount * 4, HexTools.SPACING_NONE))
                      .append(":");

                }
                else
                {
                    sb.append(HexTools.toHex(i + startAddress, hexDigitCount * 4, HexTools.SPACING_NONE))
                      .append(":");
                }
            }
    ...
    /**
     * Convert a byte to hex value with leading zeroes added
     * The output will always be 2 characters long
     * The output is the binary representation of the digit
     */
