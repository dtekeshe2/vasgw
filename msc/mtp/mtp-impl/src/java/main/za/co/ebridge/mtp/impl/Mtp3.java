...
import za.co.ebridge.stream.api.SelectorKey;
import za.co.ebridge.stream.api.SelectorProvider;
import za.co.ebridge.stream.api.StreamSelector;
        ...
        implements Runnable
{
    ...
    // protocols.ss7.cap.api has subservice as 1, q704 shows the same iirc
    ...
    public Mtp3(String name, Scheduler scheduler)
    {
        ...
        try
        {
            selector = SelectorProvider.getSelector("org.mobicents.protocols.ss7.cap.api.hardware.dahdi.Selector");
        }
        catch (Exception e)
        {
            e.printStackTrace();
     ...
     * (non-Javadoc)
     *
     * @see Mtp3#addMtp3Listener(Mtp3Listener)
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.mtp.Mtp3#removeMtp3Listener(org.mobicents.protocols
     * .protocols.ss7.cap.api.mtp.Mtp3Listener)
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.mtp.Mtp3#setLinks(java.util.List)
     */
    ...
    public void onMessage(Mtp2Buffer rxFrame, Mtp2 mtp2)
    {
        ...
        int ni = (subserviceIndicator & 0x0C); // local NI(network Indicator) form msg., 0x0C, since we store it as
        // shifted
        // value.
        ...
        if (this.ni != ni)
        {
            ...
            {
                logger.error(String.format("(%s) Received MSSU with bad SSI, discarding! ni:" + ni + " thisni:" +
                        this.ni
        ...
        switch (serviceIndicator)
        {
            ...
            default:
                if (logger.isEnabledFor(Level.WARN))
                {
                    logger.warn("Received MSU for UNKNOWN SERVICE!!!!!!!!!!!: " + Utils.dump(rxFrame.frame, rxFrame
                            .len, false));
                }
                break;
        }
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.mtp.Mtp2Listener#registerLink(org.mobicents.protocols
     * .protocols.ss7.cap.api.mtp.Mtp2)
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.mtp.Mtp2Listener#unregisterLink(org.mobicents.protocols
     * .protocols.ss7.cap.api.mtp.Mtp2)
     */
            ...
            extends Task
    {
        ...
        private byte[] sltm = new byte[7 + SLTM_PATTERN.length]; // this has to be separate, cause its async to Mtp3
        // .send
        ...
        public long perform()
        {
            ...
            switch (tryCount)
            {
                ...
                case 1:
                    if (logger.isDebugEnabled())
                    {
                        logger.debug(String.format("(%s) SLTM message was not acknowledged, Link failed", link
                                .getName()));
                    }
