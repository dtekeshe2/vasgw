...
import za.co.ebridge.scheduler.Scheduler;
import za.co.ebridge.scheduler.Task;
...
public class Mtp2
{
    ...
    private static final int[] fcstab = new int[]{0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf,
            0x8c48,
            ...
            0x1291,
            0x0318, 0x77a7, 0x662e, 0x54b5, 0x453c, 0xbdcb, 0xac42, 0x9ed9, 0x8f50, 0xfbef, 0xea66, 0xd8fd, 0xc974,
            0x4204,
            0x538d, 0x6116, 0x709f, 0x0420, 0x15a9, 0x2732, 0x36bb, 0xce4c, 0xdfc5, 0xed5e, 0xfcd7, 0x8868, 0x99e1,
            0xab7a,
            0xbaf3, 0x5285, 0x430c, 0x7197, 0x601e, 0x14a1, 0x0528, 0x37b3, 0x263a, 0xdecd, 0xcf44, 0xfddf, 0xec56,
            0x98e9,
            0x8960, 0xbbfb, 0xaa72, 0x6306, 0x728f, 0x4014, 0x519d, 0x2522, 0x34ab, 0x0630, 0x17b9, 0xef4e, 0xfec7,
            0xcc5c,
            0xddd5, 0xa96a, 0xb8e3, 0x8a78, 0x9bf1, 0x7387, 0x620e, 0x5095, 0x411c, 0x35a3, 0x242a, 0x16b1, 0x0738,
            0xffcf,
            0xee46, 0xdcdd, 0xcd54, 0xb9eb, 0xa862, 0x9af9, 0x8b70, 0x8408, 0x9581, 0xa71a, 0xb693, 0xc22c, 0xd3a5,
            0xe13e,
            0xf0b7, 0x0840, 0x19c9, 0x2b52, 0x3adb, 0x4e64, 0x5fed, 0x6d76, 0x7cff, 0x9489, 0x8500, 0xb79b, 0xa612,
            0xd2ad,
            0xc324, 0xf1bf, 0xe036, 0x18c1, 0x0948, 0x3bd3, 0x2a5a, 0x5ee5, 0x4f6c, 0x7df7, 0x6c7e, 0xa50a, 0xb483,
            0x8618,
            0x9791, 0xe32e, 0xf2a7, 0xc03c, 0xd1b5, 0x2942, 0x38cb, 0x0a50, 0x1bd9, 0x6f66, 0x7eef, 0x4c74, 0x5dfd,
            0xb58b,
            ...
            0x6b46,
            0x7acf, 0x4854, 0x59dd, 0x2d62, 0x3ceb, 0x0e70, 0x1ff9, 0xf78f, 0xe606, 0xd49d, 0xc514, 0xb1ab, 0xa022,
            0x92b9,
     ...
     * ones-density
     * violation (the data field simulated a flag) or the SIF has exceeded its maximum capacity of 272 octets. The
     * SIO is sent
     ...
     * initiated
     * the alignment procedure. The link is made unavailable for MSUs, and only FISUs are transmitted to the affected
     * signaling
     * point until a proving period has been passed. After successful completion of a proving period, MSUs can be
     * transmitted
     ...
     * transmit
     * any MSUs for reasons other than a processor outage. On receipt of an SIOS, the receiving signaling point stops
     * the
     * transmission of MSUs and begins transmitting FISUs. The SIOS is also sent at the beginning of the alignment
     * procedure.
     */
    ...
    static
    {
        STATE_NAMES = new String[]{"OUT_OF_SERVICE", "NOT_ALIGNED", "ALIGNED", "PROVING", "ALIGNED_READY",
                "IN_SERVICE"};
     ...
     * Retransmission will occur until ticks wont get to point where buffer.isFree() == true. It also point to next
     * MSU if
    ...
    /**
     * This is FSN we used as last for send SU. If we spin many times and it equals retransmissionFSN_LastAcked, it
     * means we
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.mtp.Mtp2#getName()
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.mtp.Mtp2#getSls()
     */
    public int getSls()
    {
        return this.sls;
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.mtp.Mtp2#setName(java.lang.String)
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.mtp.Mtp2#setSls(int)
     */
    ...
    private void processFrame()
    {
        ...
        if ((this.retransmissionFSN_LastAcked <= this.retransmissionFSN_LastSent && (bsn < this
                .retransmissionFSN_LastAcked || bsn > this.retransmissionFSN_LastSent))
                ||
                // CASE_II, buffer flipped, lastSent is less than ack, since it
                // starts from "0"
                (this.retransmissionFSN_LastAcked > this.retransmissionFSN_LastSent && (bsn < this
                        .retransmissionFSN_LastAcked && bsn > this.retransmissionFSN_LastSent)))
        {
            this.bsnErrors++;
            if (this.bsnErrors > 2)
    ...
    private void processTx(int bytesRead)
            throws
            IOException
    {
    ...
    private void alignmentNotPossible(String cause)
    {
        ...
        if (logger.isDebugEnabled())
        {
            logger.debug(String.format("(%s) Alignment not possible, initiating T17 for restart. Cause: %s", name,
                    cause));
        }
    }
