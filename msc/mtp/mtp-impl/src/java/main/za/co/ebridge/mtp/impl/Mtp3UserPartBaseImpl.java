...
public abstract class Mtp3UserPartBaseImpl
     ...
     *
     * @param msg
     * @param seqControl For the thread selection (for message delivering)
     */
    protected void sendTransferMessageToLocalUser(Mtp3TransferPrimitive msg, int seqControl)
    {

        logger.debug("sendTransferMessageToLocalUser payload size : " + msg.getData().length);
        if ( this.isStarted )
        {
            ...
            implements Runnable
    {
        ...
        public void run()
        {
            ...
            logger.debug("MsgTransferDeliveryHandler()");
            if (isStarted)
            {
                try
                {
                    for (Mtp3UserPartListener listener : userListeners)
                    {
                        listener.onMtp3TransferMessage(this.msg);
                    }
                }
                catch (Exception e)
                {
                    logger.error("Exception while delivering a system messages to the MTP3-user: " +
                                    e.getMessage(), e);
                }
            }
            else
            ...
            implements Runnable
    {
        ...
        public void run()
        {
                ...
                try
                {
                    for (Mtp3UserPartListener listener : userListeners)
                    {
                        if (this.msg.getType() == Mtp3Primitive.PAUSE)
                            listener.onMtp3PauseMessage((Mtp3PausePrimitive) this.msg);
                        if (this.msg.getType() == Mtp3Primitive.RESUME)
                            listener.onMtp3ResumeMessage((Mtp3ResumePrimitive) this.msg);
                        if (this.msg.getType() == Mtp3Primitive.STATUS)
                            listener.onMtp3StatusMessage((Mtp3StatusPrimitive) this.msg);
                    }
                }
                catch (Exception e)
                {
                    logger.error("Exception while delivering a payload messages to the MTP3-user: " +
                                    e.getMessage(), e);
                }
            }
            else
            {
                logger.error(String.format(
                        "Received Mtp3Primitive=%s but Mtp3UserPart is not started. Message will be dropped",
                        msg));
            }
        }
    }
