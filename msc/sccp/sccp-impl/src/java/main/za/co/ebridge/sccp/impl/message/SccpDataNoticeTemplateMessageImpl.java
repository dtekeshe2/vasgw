...
import za.co.ebridge.sccp.impl.parameter.ImportanceImpl;
        ...
        extends SccpSegmentableMessageImpl
{
    ...
    protected SccpDataNoticeTemplateMessageImpl(
    ...
    protected SccpDataNoticeTemplateMessageImpl(
            SccpStackImpl sccpStackImpl, int type, int incomingOpc, int incomingDpc,
            int incomingSls)
    {
    ...
    protected abstract byte[] getSecondParamaterData()
            throws
            IOException;

    protected abstract void setSecondParamaterData(int data)
            throws
            IOException;
            ...
            IOException
    {
        switch (this.type)
        {
            ...
            case SccpMessage.MESSAGE_TYPE_XUDTS:
            {
                ...
                if (this.hopCounter.getValue() > HopCounter.COUNT_HIGH || this.hopCounter.getValue() <= HopCounter
                        .COUNT_LOW)
                {
    ...
    private void decodeOptional(int code, byte[] buffer)
            throws
            IOException
    {
            ...
            IOException
    {
        ...
        if (this.isMtpOriginated && this.type == SccpMessage.MESSAGE_TYPE_UDT || this.type == SccpMessage
                .MESSAGE_TYPE_UDTS)
        ...
        if (longMessageRuleType == LongMessageRuleType.LongMessagesForbidden)
        {
            ...
            { // message is too long to encode UDT
                if (logger.isEnabledFor(Level.WARN))
                {
                    logger.warn(String.format("Failure when sending a UDT message: message is too long. " +
                                    "SccpMessageSegment=%s",
                            this));
                }
                return new EncodingResultData(EncodingResult.ReturnFailure, null, null, ReturnCauseValue
                        .SEG_NOT_SUPPORTED);
            }
            ...
            } else
            {
                ...
                if (bf.length > availLenXSegm * 16)
                {
                    if (logger.isEnabledFor(Level.WARN))
                    {
                        logger.warn(String.format(
                                "Failure when segmenting a message XUDT: message is too long. SccpMessageSegment=%s",
                                this));
                    }
                    return new EncodingResultData(EncodingResult.ReturnFailure, null, null, ReturnCauseValue
                            .SEG_FAILURE);
                ...
                if (this.isMtpOriginated)
                {
                    if (this.segmentation == null)
                    {
                        ...
                        {
                            logger.warn(String
                                    .format("Failure when segmenting a message: message is not locally originated but" +
                                                    " \"segmentation\" field is absent. SccpMessageSegment=%s",
                                            this));
                        }
                        return new EncodingResultData(EncodingResult.ReturnFailure, null, null, ReturnCauseValue
                                .SEG_FAILURE);
                    ...
                    this.segmentation = new SegmentationImpl(true, this.segmentation.isClass1Selected(), (byte)
                            segmCount,
        ...
        } else
        {
            ...
            if (bf.length > availLen)
            { // message is too long to encode LUDT
                if (logger.isEnabledFor(Level.WARN))
                {
                    logger.warn(String.format(
