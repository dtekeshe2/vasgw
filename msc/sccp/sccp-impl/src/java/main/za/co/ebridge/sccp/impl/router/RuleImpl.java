...
import za.co.ebridge.sccp.api.LoadSharingAlgorithm;
import za.co.ebridge.sccp.api.OriginationType;
import za.co.ebridge.sccp.api.Rule;
import za.co.ebridge.sccp.api.RuleType;
        ...
        implements Rule, Serializable
{
     ...
     */
    public RuleImpl(
            RuleType ruleType, LoadSharingAlgorithm loadSharingAlgo, OriginationType originationType,
        ...
        switch (gti)
        {
            case GLOBAL_TITLE_INCLUDES_NATURE_OF_ADDRESS_INDICATOR_ONLY:
                ...
                if (!matchPattern(gt.getDigits().toCharArray(), patternGT.getDigits().toCharArray()))
                {
                    if (logger.isInfoEnabled())
                    {
                        logger.info(String.format("digits didn't match. Pattern digits=%s Address Digits=%s Return  " +
                                        "False",
     ...
     */
    protected static final XMLFormat<RuleImpl> RULE_XML = new XMLFormat<RuleImpl>()
    {

        public void read(javolution.xml.XMLFormat.InputElement xml, RuleImpl rule)
                throws
                XMLStreamException
        {
        ...
        public void write(RuleImpl rule, javolution.xml.XMLFormat.OutputElement xml)
                throws
                XMLStreamException
        {
