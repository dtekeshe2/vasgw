        ...
        implements Dialog
{
    ...
    protected DialogImpl(
            SccpAddress localAddress, SccpAddress remoteAddress, Long origTransactionId, boolean structured,
     ...
     */
    protected DialogImpl(
            SccpAddress localAddress, SccpAddress remoteAddress, int seqControl,
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#getDialogId()
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#getNewInvokeId()
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#cancelInvocation (java.lang.Long)
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#getRemoteAddress()
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#getLocalAddress()
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#isEstabilished()
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#isStructured()
     */
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#send(org.mobicents
     * .protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.events.TCBeginRequest)
     */
    public void send(TCBeginRequest event)
            throws
            TCAPSendException
    {
        ...
        try
        {
            ...
            try
            {
                ...
                this.provider.send(aos.toByteArray(), event.getReturnMessageOnError(), this.remoteAddress, this
                                .localAddress,
                        this.seqControl, this.ssn);
     ...
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#send(org.mobicents
     * .protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.events.TCContinueRequest)
     */
    public void send(TCContinueRequest event)
            throws
            TCAPSendException
    {
        ...
        try
        {
            ...
            if (this.state == TRPseudoState.InitialReceived)
            {
                ...
                // David
                logger.debug("##### event.getApplicationContextName() " + event.getApplicationContextName());

                if (event.getApplicationContextName() != null)
                {
                    logger.debug("Adding dialog portion");
                    // set dialog portion
                    DialogPortion dp = TcapFactory.createDialogPortion();
                    dp.setUnidirectional(false);
                    ...
                    dp.setDialogAPDU(apdu);
                    apdu.setApplicationContextName(event.getApplicationContextName());
                    logger.debug("event.getUserInformation()");
                    if (event.getUserInformation() != null)
                    {
                        logger.debug("apdu.setUserInformation ( oid : " + event.getUserInformation().getOidValue());
                        apdu.setUserInformation(event.getUserInformation());
                    }
                    ...
                    apdu.setResult(res);
                    tcbm.setDialogPortion(dp);
                    logger.debug("Adding dialog portion completed");
                }

                logger.debug("setOriginatingTransactionId(" + this.localTransactionId + ")");
                tcbm.setOriginatingTransactionId(Utils.encodeTransactionId(this.localTransactionId));

                logger.debug("setDestinationTransactionId(" + this.remoteTransactionId + ")");
                tcbm.setDestinationTransactionId(this.remoteTransactionId);

                logger.debug("this.scheduledComponentList.size() : " + this.scheduledComponentList.size());
                if (this.scheduledComponentList.size() > 0)
                {
                    logger.debug("building component list.");
                    Component[] componentsToSend = new Component[this.scheduledComponentList.size()];
                    this.prepareComponents(componentsToSend);
                    tcbm.setComponent(componentsToSend);
                ...
                try
                {
                    ...
                    this.provider.send(aos.toByteArray(), event.getReturnMessageOnError(), this.remoteAddress,
                            this.localAddress, this.seqControl, this.ssn);
     ...
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#send(org.mobicents
     * .protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.events.TCEndRequest)
     */
    public void send(TCEndRequest event)
            throws
            TCAPSendException
    {
        ...
        try
        {
            ...
            if (state == TRPseudoState.InitialReceived)
            {
                ...
                if (acn != null)
                { // acn & DialogPortion is absent in TCAP V1
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#sendUni()
     */
    public void send(TCUniRequest event)
            throws
            TCAPSendException
    {
    ...
    public void send(TCUserAbortRequest event)
            throws
            TCAPSendException
    {
                    ...
                    || this.state == TRPseudoState.Active)
            {
                ...
                if (event.getUserInformation() != null || event.getDialogServiceUserType() != null)
                {
                    ...
                    } else
                    {
                        ...
                        // dialogue-refused". In such a case, a Dialogue Abort (ABRT) APDU is generated with
                ...
                try
                {
                    ...
                    this.provider.send(aos.toByteArray(), event.getReturnMessageOnError(), this.remoteAddress,
                            this.localAddress, this.seqControl, ssn);
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#sendComponent(org
     * .mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.component.ComponentRequest)
     */
    public void sendComponent(Component componentRequest)
            throws
            TCAPSendException
    {

        logger.debug("sendComponent() : this.previewMode " + this.previewMode);

        if (this.previewMode)
            return;

        logger.debug("this.provider.getStack().getStatisticsEnabled() : " +
                this.provider.getStack().getStatisticsEnabled());
        if (this.provider.getStack().getStatisticsEnabled())
        {
            switch (componentRequest.getType())
            {
                case Invoke:
                    ...
                    if (oc != null)
                    {
                        this.provider.getStack().getCounterProviderImpl().updateOutgoingInvokesPerOperationCode(oc
                                .getStringValue());
                    }
                    break;
                case ReturnResult:
                ...
                case ReturnError:
                    ...
                    if (ec != null)
                    {
                        this.provider.getStack().getCounterProviderImpl().updateOutgoingErrorsPerErrorCode(ec
                                .getStringValue());
                    }
                    break;
                case Reject:
                    ...
                    if (prob != null)
                    {
                        this.provider.getStack().getCounterProviderImpl().updateOutgoingRejectPerProblem(prob
                                .getStringValue());
                    }
                    break;
            }
        ...
        logger.debug("processing component request ...");

        try
        {
            this.dialogLock.lock();
            logger.debug(" componentRequest.getType() : " + componentRequest.getType());
            if (componentRequest.getType() == ComponentType.Invoke)
            {

                logger.debug("processing invoke request ...");
                InvokeImpl invoke = (InvokeImpl) componentRequest;

                logger.debug("checking if there is invoke request for id : " + invoke.getInvokeId() + " already.");
                // check if its taken!
                ...
                logger.debug("New invoke. Changing state to Pending.");
            ...
            {
                logger.debug("not an invoke request.");
            ...
            logger.debug("Scheduling component request with invokeID : : " + componentRequest.getInvokeId());
    ...
    private void prepareComponents(Component[] res)
    {
        ...
        while (this.scheduledComponentList.size() > index)
        {
            ...
            logger.debug("Component Type : " + cr.getType());
            ...
            if (cr.getType() == ComponentType.Invoke)
            {
                ...
                {
                    logger.warn("InvokeImpl.invokeId : " + in.getInvokeId() + " skipped. Has no provider.");
                    index++;
                    continue;
                }
    ...
    public int getDataLength(TCBeginRequest event)
            throws
            TCAPSendException
    {
    ...
    public int getDataLength(TCContinueRequest event)
            throws
            TCAPSendException
    {
    ...
    public int getDataLength(TCEndRequest event)
            throws
            TCAPSendException
    {
    ...
    public int getDataLength(TCUniRequest event)
            throws
            TCAPSendException
    {
    ...
    void processUni(TCUniMessage msg, SccpAddress localAddress, SccpAddress remoteAddress)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                TCUniIndicationImpl tcUniIndication = (TCUniIndicationImpl) ((DialogPrimitiveFactoryImpl) this
                        .provider.getDialogPrimitiveFactory())
            ...
            for (Component comp : comps)
            {
                switch (comp.getType())
                {
                    case Invoke:
                        ...
                        if (oc != null)
                        {
                            this.provider.getStack().getCounterProviderImpl().updateIncomingInvokesPerOperationCode
                                    (oc.getStringValue());
                        }
                        break;
                    case ReturnResult:
                    ...
                    case ReturnError:
                        ...
                        if (ec != null)
                        {
                            this.provider.getStack().getCounterProviderImpl().updateIncomingErrorsPerErrorCode(ec
                                    .getStringValue());
                        }
                        break;
                    case Reject:
                        ...
                        if (!rej.isLocalOriginated())
                        {
                            ...
                            if (prob != null)
                            {
                                this.provider.getStack().getCounterProviderImpl().updateIncomingRejectPerProblem(prob
                                        .getStringValue());
                            }
                        }
                        break;
    ...
    protected void processBegin(TCBeginMessage msg, SccpAddress localAddress, SccpAddress remoteAddress)
    {
        ...
        logger.debug("processBegin(TCBeginMessage msg, SccpAddress localAddress, SccpAddress remoteAddress)");
        ...
        try
        {
            ...
            logger.debug("this.previewMode : " + this.previewMode);
            if (!this.previewMode)
            {
            ...
            logger.debug("Converting Dialog to Indications");
            ...
            if (dialogPortion != null)
            {
                ...
                logger.debug("ContextNAme : " + this.lastACN + ", User Information : " + this.lastUI);
            ...
            if (this.provider.getStack().getStatisticsEnabled())
            {
                ...
                {
                    String acn = ((ApplicationContextNameImpl) tcBeginIndication.getApplicationContextName())
                            .getStringValue();
            ...
            logger.debug("tcBeginIndication.setComponents()");
        ...
        try
        {
            ...
            try
            {
                ...
                if (type == null)
                {
                    ...
                    if ((resultSourceDiagnostic != null && resultSourceDiagnostic.getDialogServiceProviderType() !=
                            null))
                    {
                        if (resultSourceDiagnostic.getDialogServiceProviderType() == DialogServiceProviderType
                ...
                if (type != null)
                {
                    ...
                    // its TC-P-Abort
                    TCPAbortIndicationImpl tcAbortIndication = (TCPAbortIndicationImpl) ((DialogPrimitiveFactoryImpl)
                            this.provider
                ...
                {
                    // its TC-U-Abort
                    TCUserAbortIndicationImpl tcUAbortIndication = (TCUserAbortIndicationImpl) (
                            (DialogPrimitiveFactoryImpl) this.provider
        ...
        try
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    this.provider.send(aos.toByteArray(), false, this.remoteAddress, this.localAddress, this
                            .seqControl, this.ssn);
                }
                catch (Exception e)
                {
    ...
    {
        logger.debug("processOperationsState()");
        ...
        for (Component ci : components)
        {
            ...
            if (ci.getType() == ComponentType.Invoke)
            {
                ...
                logger.debug("invokeId : " + invokeId + " which is linkedId");
            }
            else
            {
                invokeId = ci.getInvokeId();
                logger.debug("invokeId : " + invokeId + " which is not linkedId");
            }
            ...
            if (invokeId != null)
            {
                ...
                logger.debug("invokeId : " + invokeId + ", index : " + index);
            ...
            switch (ci.getType())
            {
                ...
                case Invoke:
                    ...
                    } else
                    {
                        ...
                        } else
                        {
                            ...
                            {
                                logger.error(String.format("Rx : %s but there is already Invoke with this invokeId",
                                        ci));
                ...
                case ReturnResult:
                    ...
                    if (invoke == null)
                    {
                        ...
                        this.addReject(resultingIndications, ci.getInvokeId(), p);
                    } else if (invoke.getInvokeClass() != InvokeClass.Class1 && invoke.getInvokeClass() !=
                            InvokeClass.Class3)
                    {
     ...
     * (non-Javadoc)
     *
     * @see org.mobicents.protocols.protocols.ss7.cap.api.tcap.api.tc.dialog.Dialog#operationEnded(
     * org.mobicents.protocols.protocols.ss7.cap.api.tcap.tc.component.TCInvokeRequestImpl)
     */
    ...
    // TC-TIMER-RESET
    public void resetTimer(Long invokeId)
            throws
            TCAPException
    {
        try
